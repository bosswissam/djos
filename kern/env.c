...
env_init(void)
{
	...
	// Mark env_free_list as empty
	env_free_list = NULL;

	// Initialize all envs, and add to free list in same order
	int32_t i;
	for (i = NENV - 1; i >= 0; i--) {
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	uintptr_t start = (uintptr_t) ROUNDDOWN(va, PGSIZE); // start from this va
	...
	for (; start < end; start += PGSIZE) {
		// Alloc page
		struct Page* pg;
		if (!(pg = page_alloc(0))) { // couldn't allocate page
			panic("region_alloc: failed to alloc at %p of len %x\n", va, len);
		} else {
			// Insert page 
			if (page_insert(e->env_pgdir, pg, (void *)start, PTE_U | PTE_W) == -E_NO_MEM) { // failed?
				panic("region_alloc: failed to alloc at %p of len %x\n", va, len);
			}
		}
	}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	if (elfhdr->e_magic != ELF_MAGIC)
		panic("load_icode: ELF not valid.\n");

	ph = (struct Proghdr *) (binary + elfhdr->e_phoff); // start prog seg
	eph = ph + elfhdr->e_phnum; // end prog seg
	...
	for (; ph < eph; ph++) {
		...
		if (ph->p_filesz > ph->p_memsz) {
			panic("load_icode: Proghdr invalid.\n");
		}
		...
		region_alloc(e, (void *) ph->p_pa, ph->p_memsz); // user r/w 

		// Copy data (in bytes) from segment
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
	// LAB 3: Your code here.
	struct Env *env;
	if (env_alloc(&env, 0) < 0) { // parent_id = 0
		return;
	}
