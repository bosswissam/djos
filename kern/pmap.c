...
boot_alloc(uint32_t n)
{
	...
	// *nextfree must be aligned to PGSIZE
	assert((uint32_t)nextfree % PGSIZE == 0);

	// Return address of next free page w/out allocating it if n = 0	
	if (n == 0) { 
		return nextfree;
	}
	
	// Round alloc size to nearest PGSIZE
	uint32_t bytesToAssign = ROUNDUP(n, PGSIZE);
	void* allocMemAddr = nextfree;

	// Fix nextfree memory ptr
	nextfree = nextfree + bytesToAssign;
...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, IOMEMBASE-KERNBASE, 0x0, PTE_W);
...
{
	if (pp->pp_ref != 0) {
		panic("page_free: %p has non-zero pp_ref\n", pp);
	}
...
{
	pte_t* pte = pgdir_walk(pgdir, va, 0); // Don't create new pgs

	// Check if page at va exists?
