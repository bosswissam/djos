...
int execv(const char *prog, const char **argv, bool fork);
...
{
	return execv(prog, argv, 1);
}

int
exec(const char *prog, const char **argv)
{
	return execv(prog, argv, 0);
}

// Will fork off as new env if fork = 1, else replaces
// current environment
int 
execv(const char *prog, const char **argv, bool fork)
{
	...
	if (fork) {
		...
		// Replace current env with the new child env
		if ((r = sys_env_swap(child)) < 0)
			panic("sys_env_swap: %e", r);
...
// Exec, taking command-line arguments array directly on stack.
int
